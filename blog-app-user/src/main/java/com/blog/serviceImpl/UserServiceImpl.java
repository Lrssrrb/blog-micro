...
public class UserServiceImpl implements UserService {
	...
	public UserDto updateUser(UserDto userDto, Integer userid) {
		...
		if(!isAdmin().isAdmin() || !user.getEmail().equals(currentUser))
	...
	public UserAdmin isAdmin() {
		...
		if(currentUser.equals("anonymousUser")) {
			return new UserAdmin(null,false,now);
		}
		...
		for (Iterator<? extends GrantedAuthority> iterator = authorities.iterator(); iterator.hasNext();) {
			...
			if(grantedAuthority.getAuthority().equals("ADMIN_USER"))
				return new UserAdmin(currentUser,true,now);
;		}
		
		return new UserAdmin(currentUser,false,now);
	}
	...
	public UserDto findByEmail(String email) {
		...
		if(findByEmail.isEmpty())
			throw new UserExeption("No user found by email.");
		UserDto dto = this.modelMapper.map(findByEmail.get(), UserDto.class);
		return dto;
	}
